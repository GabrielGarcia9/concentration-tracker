...
from src.notifications.notification import getSondNotifyEndConcentration  # Importar notificaciones
from src.timer.timer import TemporizadorTimer  # Importar el temporizador
from src.ui.print_menu import print_menu_general, print_menu_concetracion,  print_menu_estudio, print_menu_meditacion, print_menu_ejercicio_fisico, print_menu_trabajo, print_despedida 
from src.utils.utils import generate_grouped_data, generate_combined_line_plots, generate_subgroup_plots  # Importar funciones utilitarias
...
def main():
    ...
    while not Fin:
        ...
        if opc == 1:
            ...
            config_manager = ConfigManager()
            print(f"El tiempo de estudio actual es de {config_manager.config['study_duration']/60} minutos.")
            print(f"El tiempo de meditación actual es de {config_manager.config['meditation_duration']/60} minutos.")
            print(f"El tiempo de ejercicio físico actual es de {config_manager.config['exercise_physical_duration']/60} minutos.")
            print(f"El tiempo de trabajo actual es de {config_manager.config['work_duration']/60} minutos.")
